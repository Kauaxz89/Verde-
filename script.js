 /* ============================================================
       ...
       Armazenamento persistente via window.storage
       ============================================================ */
    ...
    async function storageSet(key, value) {
      ...
      catch (e) { console.error('storage set failed', key, e); showToast('Não foi possível salvar. Tente novamente.'); }
    }
  ...
  `;
    ...
    async function loadPhotoThumbs() {
      ...
      for (const img of imgs) {
        ...
        try {
          const r = await window.storage.get('photo-data:' + id, false);
          if (r) img.src = JSON.parse(r.value);
        } catch (e) { }
      }
    }
    ...
    function detailCrescimento(p) {
      ...
      const chartW = 560, chartH = 160, pad = 28;
      let pathD = '', points = '';
      if (heights.length > 1) {
        const step = (chartW - 2 * pad) / (heights.length - 1);
        logs.filter(l => l.height).forEach((l, i) => {
          const x = pad + i * step;
          const y = chartH - pad - ((Number(l.height) / maxH) * (chartH - 2 * pad));
          pathD += (i === 0 ? 'M' : 'L') + x + ',' + y + ' ';
          points += `<circle cx="${x}" cy="${y}" r="3.5" fill="var(--leaf)"><title>${fmtDate(l.date)}: ${l.height}cm</title></circle>`;
        });
      }
      return `
  <div class="card chart-wrap">
    ...
    ${heights.length > 1 ? `<svg viewBox="0 0 ${chartW} ${chartH}" style="width:100%;height:160px;margin-top:12px;">
      <line x1="${pad}" y1="${chartH - pad}" x2="${chartW - pad}" y2="${chartH - pad}" stroke="var(--border)"/>
      <path d="${pathD}" fill="none" stroke="var(--leaf)" stroke-width="2"/>
      ${points}
